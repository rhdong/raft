...
/**
 * @brief Brute Force search with filter using the constructed index.
 *
 * @tparam       T data element type
 * @tparam       IdxT type of the indices
 * @tparam       bitmap_t       The data type of the elements in the filter of bitmap view.
 *
 * @param[in] res raft resources
 * @param[in] params configure the search
 * @param[in] idx brute force index
...
void search_with_filtering(raft::resources const& res,
                           ...
                           raft::device_matrix_view<T, int64_t, row_major> distances)
{
  raft::neighbors::detail::brute_force_search<T, IdxT, bitmap_t>(
    res, idx, queries, filter, neighbors, distances);
}
