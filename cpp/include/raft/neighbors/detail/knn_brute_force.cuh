...
void brute_force_search(
  ...
  raft::core::bitmap_view<const BitmapT, IdxT> filter,
  ...
  std::optional<raft::device_vector_view<const T, IdxT>> query_norms = std::nullopt)
{
  ...
  auto csr_view = make_device_csr_matrix_view<T, IdxT, IdxT, IdxT>(csr.get_elements().data(),
                                                                   csr.structure_view());
  dump_idx_kernel<IdxT, IdxT><<<1, 1, 0, stream>>>((IdxT*)(csr_view.get_indices().data()), IdxT(csr_view.get_indices().size()),"d get_indices"); 
  dump_idx_kernel<IdxT, IdxT><<<1, 1, 0, stream>>>((IdxT*)(csr_view.get_indptr().data()), IdxT(csr_view.get_indptr().size()),"d get_indptr"); 
